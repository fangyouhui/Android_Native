package com.gh.qiniushortvideo.view.layer;
...
public class StickerTextView extends PLTextView {
    ...
    /**
     * View的宽度和高度，随着文字的旋转而变化(不包括控制旋转，缩放图片的宽高)
     */
    ...
    private boolean mIsTextSelectable = true;
    ...
    private void obtainStyledAttributes(AttributeSet attrs) {
        ...
        mFramePadding = mTypedArray.getDimensionPixelSize(R.styleable.StickerView_framePadding,
                DEFAULT_FRAME_PADDING);
        mFrameWidth = mTypedArray.getDimensionPixelSize(R.styleable.StickerView_frameWidth,
                DEFAULT_FRAME_WIDTH);
        ...
        mControlLocation = mTypedArray.getInt(R.styleable.StickerView_controlLocation,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIsEditable) {
            ...
            mControlDrawable.setBounds(mControlPoint.x - mControlDrawableWidth / 2,
                    mControlPoint.y - mControlDrawableHeight / 2,
            ...
            if (mEditDrawable != null && mShowEdit) {
                mEditDrawable.setBounds(mEditPoint.x - mControlDrawableWidth / 2,
                        mEditPoint.y - mControlDrawableHeight / 2, mEditPoint.x + mControlDrawableWidth / 2
                        , mEditPoint.y + mControlDrawableHeight / 2);
            ...
            if (mDeleteDrawable != null && mShowDelete) {
                mDeleteDrawable.setBounds(mDeletePoint.x - mControlDrawableWidth / 2,
                        mDeletePoint.y - mControlDrawableHeight / 2,
                        mDeletePoint.x + mControlDrawableWidth / 2,
                        mDeletePoint.y + mControlDrawableHeight / 2);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mStatus == STATUS_DRAG && mDownX == event.getX() && mDownY == event.getY() && mClickListener != null) {
                    // 响应点击事件
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mStatus == STATUS_ROTATE_ZOOM) {
                    // 文字对角线一半的距离
                    float halfTextDiagonal =
                            (float) Math.sqrt(Math.pow(mTextWidth / 2, 2) + Math.pow(mTextHeight / 2, 2));
                    // 当前的点到 view 中心的距离
                    float curPointToViewCenterDistance = Utils.distance4PointF(mCenterPoint, mCurMovePointF);
                    // 之前的点到 view 中心的距离
                    float prePointToCenterDistance = Utils.distance4PointF(mCenterPoint, mPreMovePointF);
                    // 计算缩放幅度
                    float changeScale =
                            (curPointToViewCenterDistance - prePointToCenterDistance) * 2 / halfTextDiagonal;
                    mScale += changeScale;
                    // 缩放比例的界限判断
                    if (mScale <= MIN_SCALE) {
                        mScale = MIN_SCALE;
    ...
    private void transformDraw() {
        ...
        mTextHeight = mTextPaint.descent() - mTextPaint.ascent();
        // 计算旋转后的矩形位置和宽高
        computeRect(mFramePadding, mFramePadding, (int) mTextWidth + mFramePadding,
                (int) mTextHeight + mFramePadding, mDegree);
        // 调整布局
        adjustLayout();
    }
    ...
    private void computeRect(int left, int top, int right, int bottom, float degree) {
        ...
        // 将 Bitmap 的四个点的X的坐标移动 mOffsetX + halfDrawableWidth
    ...
    private int judgeStatus(float x, float y) {
        ...
        // 点击的点到控制旋转，缩放点的距离
        float disToControl = Utils.distance4PointF(touchPoint, new PointF(mControlPoint));
    ...
    public void setFramePadding(int framePadding) {
        ...
        this.mFramePadding = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, framePadding,
                mDisplayMetrics);
        transformDraw();
    }
    ...
    public void setFrameWidth(int frameWidth) {
        ...
        this.mFrameWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, frameWidth,
                mDisplayMetrics);
