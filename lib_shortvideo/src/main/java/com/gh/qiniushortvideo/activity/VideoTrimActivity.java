...
import com.gh.qiniushortvideo.view.VideoFrameListView;
import com.gh.qiniushortvideo.view.crop.CropVideoView;
...
import static com.gh.qiniushortvideo.activity.VideoMixRecordActivity.VIDEO_PATH;
...
public class VideoTrimActivity extends AppCompatActivity implements
    ...
    public void onClickTrim(View v) {
        ...
        mShortVideoTrimmer.trim(mSelectedBeginMs, mSelectedEndMs, PLShortVideoTrimmer.TRIM_MODE.FAST,
                new PLVideoSaveListener() {
                    @Override
                            ...
                            public void run() {
                                if (isCroppingVideo() || mRotation != 0) {
                                    mShortVideoTranscoder =
                                            new PLShortVideoTranscoder(VideoTrimActivity.this, path
                                            , Config.TRANSCODE_FILE_PATH);
                                    ...
                                    mShortVideoTranscoder.transcode(dstWidth, dstHeight,
                                            mMediaFile.getVideoBitrate(), mRotation,
                                            new PLVideoSaveListener() {
                                                ...
                                                public void onSaveVideoSuccess(final String path) {
                                                    ...
                                                    runOnUiThread(new Runnable() {
                                                        ...
                                                        public void run() {
                                                            ...
                                                            VideoEditActivity.start(VideoTrimActivity.this,
                                                                    path);
                                                        }
                                                    });
                                                }
                    ...
                    public void onProgressUpdate(final float percentage) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                mProcessingDialog.setProgress((int) ((isCroppingVideo() || mRotation != 0) ?
                                        100 * percentage / 2 : 100 * percentage));
                            }
                        });
                    }
                });
    ...
    private void init(String videoPath) {
        ...
        mShortVideoTranscoder = new PLShortVideoTranscoder(VideoTrimActivity.this, videoPath,
                Config.TRANSCODE_FILE_PATH);
        ...
        mVideoFrameListView.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                mLoadFrameTask = new LoadFrameTask(VideoTrimActivity.this, mMediaFile, FRAME_COUNT,
                        sliceEdge, sliceEdge, new LoadFrameTask.OnLoadFrameListener() {
                    @Override
