package com.gh.qiniushortvideo.utils;
...
import com.gh.qiniushortvideo.model.MediaFile;
...
public class MediaUtils {
    ...
    public static void getLocalMedia(final Context context, final int mediaType,
                                     final LocalMediaCallback localMediaCallback) {
        ThreadPoolUtils.getExecutor().execute(new Runnable() {
            ...
            public void run() {
                ...
                if (mediaType == MEDIA_TYPE_VIDEO || mediaType == MEDIA_TYPE_ALL) {
                    ...
                    String[] whereArgs = {"video/mp4", "video/3gp", "video/flv", "video/mkv", "video/mov",
                            "video/mpg"};
                ...
                int totalMediaCount =
                        (videoCursor == null ? 0 : videoCursor.getCount()) + (imageCursor == null ? 0 :
                ...
                for (int i = 0; i < totalMediaCount; i++) {
                    if (videoCursor != null) {
                        while (videoFile == null && videoMoveToNext && videoCursor.moveToNext()) {
                            videoFile = generateVideoFile(context, videoCursor, colVideoId, colVideoData,
                                    colVideoDuration, colVideoDisplayName, colVideoAddTime);
                        }
                    }
                    if (imageCursor != null) {
                        while (imageFile == null && imageMoveToNext && imageCursor.moveToNext()) {
                            imageFile = generateImageFile(context, imageCursor, colImageId, colImageData,
                                    colImageDisplayName, colImageAddTime);
                        }
                    }
    ...
    private static MediaFile generateVideoFile(Context context, Cursor cursor, int colId, int colData,
                                               int colDuration, int colDisplayName, int colAddTime) {
        ...
        Cursor thumbCursor =
                context.getContentResolver().query(MediaStore.Video.Thumbnails.EXTERNAL_CONTENT_URI,
                        new String[]{
        ...
        return new MediaFile(videoId, MediaFile.VIDEO, filePath, thumbnailPath == null ? filePath :
                thumbnailPath, duration, displayName, addTime);
    }

    private static MediaFile generateImageFile(Context context, Cursor cursor, int colId, int colData,
                                               int colDisplayName, int colAddTime) {
        String filePath = cursor.getString(colData);
        if (!new File(filePath).exists()) {
            return null;
        }
        ...
        return new MediaFile(imageId, MediaFile.IMAGE, filePath, thumbnailPath == null ? filePath :
                thumbnailPath, displayName, addTime);
    }
}
