package com.gh.qiniushortvideo.tusdk.playview;
...
import com.gh.qiniushortvideo.tusdk.playview.rangeselect.TuSdkMovieGrayView;
                ...
                case MotionEvent.ACTION_CANCEL:
                    if (touchType == 0) {
                        ThreadHelper.post(new Runnable() {
                            ...
                            public void run() {
                                if (onTouchSelectBarListener != null) {
                                    onTouchSelectBarListener.onTouchBar(mLastStarPercent, mLastEndPercent,
                                            touchType);
                                }
                            }
                        });
                        ...
                        ThreadHelper.post(new Runnable() {
                            ...
                            public void run() {
                                float stopX =
                                        mCenterFrame.getX() + mCenterFrame.getWidth() - mRightGrayView.getBarWidth();
                                if (onTouchSelectBarListener != null) {
                                    onTouchSelectBarListener.onTouchBar(mLastStarPercent, mLastEndPercent,
                                            touchType);
                                }
...
//                }
    ...
    private boolean handleActionDownEvent(MotionEvent event) {
        oldX = (int) event.getX();
        //触摸左边视图
        boolean isTouchLeftBarRight =
                0 <= Math.abs(oldX - mLeftGrayView.getRight()) && 40 >= Math.abs(oldX - mLeftGrayView.getRight());
        boolean isTouchLeftBarLeft =
                0 <= Math.abs(oldX - (mLeftGrayView.getRight() - mLeftGrayView.getBarWidth())) && 40 >= Math.abs(oldX - (mLeftGrayView.getRight() - mLeftGrayView.getBarWidth()));
        ...
        boolean isTouchRightBarRight =
                0 <= Math.abs(oldX - mRightGrayView.getLeft()) && 40 >= Math.abs(oldX - mRightGrayView.getLeft());
        boolean isTouchRightBarLeft =
                0 <= Math.abs(oldX - (mRightGrayView.getLeft() + mRightGrayView.getBarWidth())) && 40 >= Math.abs(oldX - (mRightGrayView.getLeft() + mRightGrayView.getBarWidth()));
        //触摸的右边视图
    ...
    private void moveRightBar(int distance) {
        ...
        if (mSelectRangeChangedListener != null)
            mSelectRangeChangedListener.onSelectRangeChanged(getFormatPerchent(startPercent),
                    getFormatPerchent(endPercent), 1);
    ...
    private void moveLeftBar(int distance) {
        ...
        if (mSelectRangeChangedListener != null)
            mSelectRangeChangedListener.onSelectRangeChanged(getFormatPerchent(startPercent),
                    getFormatPerchent(endPercent), 0);
