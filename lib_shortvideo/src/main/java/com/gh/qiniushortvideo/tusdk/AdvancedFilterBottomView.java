...
//        getEditorController().getMovieEditor().getEditorEffector().addMediaEffectData(mSelectEffectData);
    ...
    protected void init() {
        ...
        mFilterRecyclerView.setLayoutManager(new LinearLayoutManager(mContext,
    ...
    public void onFilterItemClicked(int position) {
        ...
        if (mOnFilterClickedListener != null) {
            mOnFilterClickedListener.onFilterClicked(position,
                    mAdvancedFilterAdapter.getFilterList().get(mSelectIndex));
        }
    }
}
