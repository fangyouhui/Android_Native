package com.gh.qiniushortvideo.tusdk.playview.rangeselect;
...
public class TuSdkMovieColorGroupView extends FrameLayout {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (rectView.getDrawDirection() == 0) {
                ...
                rectView.layout(startPosition, top, startPosition + rectView.getMeasuredWidth() + diff,
                        bottom);
            } else {
                int diff = 0;
                if (i > 0 && getChildAt(i - 1) != null && getChildAt(i - 1).getVisibility() == VISIBLE) {
                ...
                rectView.layout(startPosition - rectView.getMeasuredWidth() - diff, top, startPosition,
                        bottom);
            }
        }
    }
    ...
    public TuSdkMovieColorRectView getPointColorRect(float pointX, float pointY) {
        LinkedList<TuSdkMovieColorRectView> reverList =
                (LinkedList<TuSdkMovieColorRectView>) mColorRectList.clone();
