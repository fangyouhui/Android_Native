...
public class FURenderer implements OnFUControlListener {
    ...
    public void onSurfaceCreated() {
        ...
        if (mDefaultEffect != null) {
            mFuItemHandler.sendMessage(Message.obtain(mFuItemHandler, ITEM_ARRAYS_EFFECT_INDEX,
                    mDefaultEffect));
        }
    ...
    public void onCartoonFilterSelected(final int style) {
        ...
        if (mFuItemHandler == null) {
            queueEvent(new Runnable() {
                @Override
                public void run() {
                    mFuItemHandler.sendMessage(Message.obtain(mFuItemHandler,
                            ITEM_ARRAYS_FUZZYTOON_FILTER_INDEX, mComicFilterStyle));
                }
            });
        } else {
            mFuItemHandler.sendMessage(Message.obtain(mFuItemHandler, ITEM_ARRAYS_FUZZYTOON_FILTER_INDEX,
                    mComicFilterStyle));
        }
    }
     ...
     * @return
     */
    public int onDrawFrame(byte[] img, int tex, int w, int h, byte[] readBackImg, int readBackW,
                           int readBackH) {
    ...
    public int onDrawFrameAvatar(byte[] img, int w, int h) {
        ...
        int tex = faceunity.fuAvatarToTexture(AvatarConstant.PUP_POS_DATA0, AvatarConstant.EXPRESSIONS0,
                AvatarConstant.ROTATION_DATA0, AvatarConstant.ROTATION_MODE_DATA0, 0, w, h, mFrameId++,
                mItemsArray,
...
//        Log.d(TAG, "recomputeFaceup() called");
    ...
    public void fuItemSetParamFaceup(final String key, final double value) {
        queueEvent(new Runnable() {
            @Override
            public void run() {
//                Log.d(TAG, "fuItemSetParamFaceup() called , key:" + key + ", value:" + value + ", handle
//                " + mItemsArray[ITEM_ARRAYS_EFFECT_INDEX]);
                faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_EFFECT_INDEX], "{\"name\":\"facepup\"," +
                        "\"param\":\"" + key + "\"}", value);
            }
        });
    }
    ...
    public float fuItemGetParamFaceup(final String key) {
        return (float) faceunity.fuItemGetParam(mItemsArray[ITEM_ARRAYS_EFFECT_INDEX], "{\"name\":\"facepup" +
                "\",\"param\":\"" + key + "\"}");
    }
...
//        Log.d(TAG, "fuItemSetParamFaceColor() called with: key = [" + key + "], value = [" + Arrays
//        .toString(value) + "]");
        queueEvent(new Runnable() {
            @Override
            public void run() {
    ...
    public void setIsFrontCamera(final boolean isFront) {
        queueEvent(new Runnable() {
            @Override
            public void run() {
                faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_LIVE_PHOTO_INDEX], "is_front", isFront ?
                        1 : 0);
            }
        });
    }
    ...
    private void prepareDrawFrame() {
        ...
        if (isNeedUpdateFaceBeauty && mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX] != 0) {
            ...
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "filter_level",
                    mFilterLevel);
            //filter_name 滤镜
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "filter_name",
            ...
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "blur_level",
                    6 * mBlurLevel);
...
//          faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_], "blur_blend_ratio", 1);
            ...
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "tooth_whiten",
                    mToothWhiten);


            //face_shape_level 美型程度 范围0~1 SDK默认为1
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "face_shape_level",
                    mFaceShapeLevel);
            ...
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "eye_enlarging",
                    mEyeEnlarging);
            //cheek_thinning 瘦脸 范围0~1 SDK默认为 0
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "cheek_thinning",
                    mCheekThinning);
            //intensity_chin 下巴 范围0~1 SDK默认为 0.5    大于0.5变大，小于0.5变小
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "intensity_chin",
                    mIntensityChin);
            //intensity_forehead 额头 范围0~1 SDK默认为 0.5    大于0.5变大，小于0.5变小
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "intensity_forehead",
                    mIntensityForehead);
            //intensity_nose 鼻子 范围0~1 SDK默认为 0
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "intensity_nose",
                    mIntensityNose);
            //intensity_mouth 嘴型 范围0~1 SDK默认为 0.5   大于0.5变大，小于0.5变小
            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_BEAUTY_INDEX], "intensity_mouth",
                    mIntensityMouth);
    ...
    @Override
    public void onPosterTemplateSelected(final int tempWidth, final int tempHeight, final byte[] temp,
        ...
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_CHANGE_FACE_INDEX], "template_face_points",
                posterTemplateLandmark);
        faceunity.fuCreateTexForItem(mItemsArray[ITEM_ARRAYS_CHANGE_FACE_INDEX], "tex_template", temp,
    ...
    public void onHairLevelSelected(@HairType final int type, int hairColorIndex, float hairColorLevel) {
        ...
        queueEvent(new Runnable() {
            ...
            public void run() {
                if (type == HAIR_NORMAL) {
                    faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_EFFECT_HAIR_NORMAL_INDEX], "Index",
                            mHairColorIndex);
                    faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_EFFECT_HAIR_NORMAL_INDEX], "Strength",
                            mHairColorStrength);
                } else if (type == HAIR_GRADIENT) {
                    faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_EFFECT_HAIR_GRADIENT_INDEX], "Index",
                            mHairColorIndex);
                    faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_EFFECT_HAIR_GRADIENT_INDEX], "Strength"
                            , mHairColorStrength);
                }
            }
        });
    ...
    @Override
    public void onPosterInputPhoto(final int inputWidth, final int inputHeight, final byte[] input,
                                   final float[] landmark) {
        ...
        faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_CHANGE_FACE_INDEX], "input_face_points",
                posterPhotoLandmark);
        faceunity.fuCreateTexForItem(mItemsArray[ITEM_ARRAYS_CHANGE_FACE_INDEX], "tex_input", input,
            ...
            queueEvent(new Runnable() {
                ...
                public void run() {
                    faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_MAKEUP_INDEX],
                            getMakeupIntensityKeyByType(integer), 0);
                }
            });
        }
    ...
    public void onMakeupSelected(final MakeupItem makeupItem, float level) {
        ...
        if (mFuItemHandler == null) {
            queueEvent(new Runnable() {
                @Override
                public void run() {
                    mFuItemHandler.sendMessage(Message.obtain(mFuItemHandler, ITEM_ARRAYS_FACE_MAKEUP_INDEX
                            , makeupItem));
                }
            });
        } else {
    ...
    public void onMakeupLevelChanged(final int makeupType, final float level) {
        ...
        queueEvent(new Runnable() {
            ...
            public void run() {
                faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_MAKEUP_INDEX],
                        getMakeupIntensityKeyByType(makeupType), level);
            }
        });
    }
    ...
    public void onMakeupOverallLevelChanged(final float level) {
        ...
        for (final Map.Entry<Integer, MakeupItem> entry : entries) {
            queueEvent(new Runnable() {
                ...
                public void run() {
                    faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FACE_MAKEUP_INDEX],
                            getMakeupIntensityKeyByType(entry.getKey()), level);
    ...
    public void onLightMakeupSelected(final MakeupItem makeupItem, final float level) {
        ...
        if (mFuItemHandler == null) {
            queueEvent(new Runnable() {
                ...
                public void run() {
                    mFuItemHandler.sendMessage(Message.obtain(mFuItemHandler,
                            ITEM_ARRAYS_LIGHT_MAKEUP_INDEX, makeupItem));
                }
            });
        } else {
            mFuItemHandler.sendMessage(Message.obtain(mFuItemHandler, ITEM_ARRAYS_LIGHT_MAKEUP_INDEX,
                    makeupItem));
        }
    }
    ...
    public void setMagicPhoto(final MagicPhotoEntity magicPhotoEntity) {
        if (mFuItemHandler == null) {
            queueEvent(new Runnable() {
                @Override
                public void run() {
                    mFuItemHandler.sendMessage(Message.obtain(mFuItemHandler, ITEM_ARRAYS_LIVE_PHOTO_INDEX,
                            magicPhotoEntity));
                }
            });
        } else {
    ...
    private int loadItem(String bundlePath) {
        ...
        try {
            if (!TextUtils.isEmpty(bundlePath)) {
                InputStream is = bundlePath.startsWith(Constant.filePath) ?
                        new FileInputStream(new File(bundlePath)) : mContext.getAssets().open(bundlePath);
    ...
    private void benchmarkFPS() {
        ...
        if (++mCurrentFrameCnt == TIME) {
            ...
            double fps =
                    (1000.0f * NANO_IN_ONE_MILLI_SECOND / ((tmp - mLastOneHundredFrameTimeStamp) / TIME));
    ...
    private void updateEffectItemParams(Effect effect, final int itemHandle) {
        ...
        if (effectType == Effect.EFFECT_TYPE_AVATAR) {
            ...
            faceunity.fuItemSetParam(itemHandle, "is3DFlipH",
                    mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_BACK ? 1 : 0);
        }
        if (effectType == Effect.EFFECT_TYPE_ANIMOJI || effectType == Effect.EFFECT_TYPE_PORTRAIT_DRIVE) {
            ...
            faceunity.fuItemSetParam(itemHandle, "isFlipExpr",
                    mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_BACK ? 1 : 0);
        ...
        if (effectType == Effect.EFFECT_TYPE_GESTURE) {
            ...
            faceunity.fuItemSetParam(itemHandle, "is3DFlipH",
                    mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_BACK ? 1 : 0);
            faceunity.fuItemSetParam(itemHandle, "loc_y_flip",
                    mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_BACK ? 1 : 0);
            faceunity.fuItemSetParam(itemHandle, "loc_x_flip",
                    mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_BACK ? 1 : 0);
            faceunity.fuItemSetParam(itemHandle, "rotMode", mRotMode);
        }
        if (effectType == Effect.EFFECT_TYPE_ANIMOJI) {
            // 设置人转向的方向
            faceunity.fuItemSetParam(itemHandle, "isFlipTrack",
                    mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_BACK ? 1 : 0);
            // 设置 Animoji 跟随人脸
...
//--------------------------------------Builder----------------------------------------
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                            ...
                            queueEventItemHandle(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    if (mLipStickColor != null) {
                                        if (makeupItem.getType() == FaceMakeup.FACE_MAKEUP_TYPE_LIPSTICK) {
                                            faceunity.fuItemSetParam(itemHandle, "makeup_lip_color",
                                                    mLipStickColor);
                                    ...
                                    if (makeupItemBytes != null) {
                                        faceunity.fuCreateTexForItem(itemHandle, key, makeupItemBytes,
                ...
                case ITEM_ARRAYS_EFFECT_HAIR_NORMAL_INDEX: {
                    ...
                    queueEventItemHandle(new Runnable() {
                        ...
                        public void run() {
                            ...
                            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_EFFECT_HAIR_NORMAL_INDEX],
                                    "Index", mHairColorIndex);
                            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_EFFECT_HAIR_NORMAL_INDEX],
                                    "Strength", mHairColorStrength);
                        }
                    });
                }
                break;
                ...
                case ITEM_ARRAYS_EFFECT_HAIR_GRADIENT_INDEX: {
                    ...
                    queueEventItemHandle(new Runnable() {
                        ...
                        public void run() {
                            ...
                            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_EFFECT_HAIR_GRADIENT_INDEX],
                                    "Index", mHairColorIndex);
                            faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_EFFECT_HAIR_GRADIENT_INDEX],
                                    "Strength", mHairColorStrength);
                        }
                    ...
                    if (style >= 0) {
                        ...
                        queueEventItemHandle(new Runnable() {
                            ...
                            public void run() {
                                ...
                                faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FUZZYTOON_FILTER_INDEX],
                                        "style", style);
                                int supportGLVersion = GlUtil.getSupportGLVersion(mContext);
                                faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_FUZZYTOON_FILTER_INDEX],
                                        "glVer", supportGLVersion);
                            }
                        });
                    } else {
                        // 关闭
                ...
                case ITEM_ARRAYS_LIVE_PHOTO_INDEX: {
                    ...
                    Bitmap bitmap = BitmapUtil.decodeSampledBitmapFromFile(magicPhotoEntity.getImagePath(),
                            magicPhotoEntity.getWidth(), magicPhotoEntity.getHeight());
                    final byte[] bytes = BitmapUtil.loadPhotoRGBABytes(bitmap);
                    if (bytes != null) {
                        queueEventItemHandle(new Runnable() {
                            @Override
                            public void run() {
                                faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_LIVE_PHOTO_INDEX],
                                        "target_width", magicPhotoEntity.getWidth());
                                faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_LIVE_PHOTO_INDEX],
                                        "target_height", magicPhotoEntity.getHeight());
                                faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_LIVE_PHOTO_INDEX],
                                        "group_type", magicPhotoEntity.getGroupType());
                                faceunity.fuItemSetParam(mItemsArray[ITEM_ARRAYS_LIVE_PHOTO_INDEX],
                                        "group_points", magicPhotoEntity.getGroupPoints());
                                faceunity.fuDeleteTexForItem(mItemsArray[ITEM_ARRAYS_LIVE_PHOTO_INDEX],
                                        "tex_input");
                                faceunity.fuCreateTexForItem(mItemsArray[ITEM_ARRAYS_LIVE_PHOTO_INDEX],
                                        "tex_input", bytes, magicPhotoEntity.getWidth(),
                                        magicPhotoEntity.getHeight());
                            }
                        });
                    }
                }
    ...
    public int onDrawFrameByFBO(byte[] cameraNV21Byte, int texId, int texWidth, int texHeight) {
        ...
        if (mCurrentCameraType == Camera.CameraInfo.CAMERA_FACING_FRONT) {
            ...
            GLES20.glViewport(originalViewPort[0], originalViewPort[1], originalViewPort[2],
                    originalViewPort[3]);
...
//Attach render buffer to depth attachment
                GLES20.glFramebufferRenderbuffer(GLES20.GL_FRAMEBUFFER, GLES20.GL_DEPTH_ATTACHMENT,
                        GLES20.GL_RENDERBUFFER, renderBufferId[i]);
//we are done, reset
