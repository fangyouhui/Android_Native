...
public class NavUtils {
    ...
    public static LatLng GCJ02ToBD09(LatLng latLng) {
        double x_pi = 3.14159265358979324 * 3000.0 / 180.0;
        double z = Math.sqrt(latLng.longitude * latLng.longitude + latLng.latitude * latLng.latitude) +
     ...
     * @param dname   终点名称 必填
     */
    public static void openGaoDeNavi(Context context, double slat, double slon, String sname, double
            dlat, double dlon, String dname) {
...
//            }
     ...
     * @param dname   终点名称 必填
     */
    public static void openBaiDuNavi(Context context, double slat, double slon, String sname, double
            dlat, double dlon, String dname) {
        String uriString = null;
        //终点坐标转换
                ...
                .setItems(R.array.nav, (dialog, which) -> {
                    if (which == 0) { // 导航应用1高德
                        if (isGdMapInstalled()) {
                            ...
                            new AlertDialog.Builder(context)
                                    ...
                                    .setPositiveButton("下载",
                                            (dialog12, which12) -> context.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(NavUtils.DOWNLOAD_GAODE_MAP))))
                        ...
                        if (isBaiduMapInstalled()) {
                            ...
                            new AlertDialog.Builder(context)
                                    .setMessage("下载百度地图？")
                                    .setPositiveButton("下载", (dialog1, which1) -> context.startActivity(new
                                            Intent(Intent.ACTION_VIEW,
                                            Uri.parse(NavUtils.DOWNLOAD_BAIDU_MAP))))
